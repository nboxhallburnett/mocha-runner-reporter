...
module.exports = class Runner {
	...
	constructor (files, avoided, params, attempts) {
		...
		// If the first argument is not a path/file, then remove it.
		if (files[0] === process.argv[2] && files[0].startsWith('-')) {
			files[0] = null;
		}
	...
	_resetMocha () {
		this.mocha = new Mocha({
			...
			timeout:	args.t || args.timeout	|| 2000,
			slow:		args.s || args.slow		|| 75,
